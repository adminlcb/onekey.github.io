;(function () {
	if (!('serial' in navigator)) {
		alert('当前浏览器不支持设备操作,请更换Edge或Chrome浏览器')
	}
	let serialPort = null
	...
	document.getElementById('serial-select-port').addEventListener('click', async () => {
		...
		try {
			...
			console.error('获取设备权限出错' + e.toString())
		}
	})
